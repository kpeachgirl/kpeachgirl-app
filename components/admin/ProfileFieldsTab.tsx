    ...
    <div style={{ display: 'grid', gap: 20 }}>
      ...
      <div style={panelStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4, flexWrap: 'wrap', gap: 8 }}>
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--charcoal)' }}>{t.pfAgeGateTitle}</h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 14, color: 'var(--muted)' }}>{t.pfAgeGateEnabled}</span>
            <button
        ...
        <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 20 }}>{t.pfAgeGateDesc}</p>
      ...
      <div style={panelStyle}>
        <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--charcoal)', marginBottom: 4 }}>{t.pfHeroTitle}</h3>
        <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 20 }}>{t.pfHeroDesc}</p>
          ...
          {heroConfig.img ? (
            <>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--charcoal)', pointerEvents: 'none' }}>
                <div style={{ fontFamily: 'var(--font-serif)', fontSize: 34, fontWeight: 300, textAlign: 'center', padding: '0 16px' }}>
                  {heroConfig.titleLine1} {heroConfig.titleLine2} <span style={{ color: 'var(--rose)', fontStyle: 'italic' }}>{heroConfig.titleAccent}</span>
                </div>
                <div style={{ fontSize: 15, color: 'var(--muted)', marginTop: 4 }}>{heroConfig.subtitle}</div>
              </div>
              {/* Bottom controls */}
              <div style={{ position: 'absolute', bottom: 12, right: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <button onClick={(e) => { e.stopPropagation(); setEditingHeroPhoto(true) }} style={{ padding: '6px 14px', background: 'rgba(0,0,0,0.6)', border: '1px solid var(--sand)', borderRadius: 6, color: 'var(--charcoal)', fontSize: 14, cursor: 'pointer' }}>{t.editBtn}</button>
                <button onClick={(e) => { e.stopPropagation(); heroFileRef.current?.click() }} style={{ padding: '6px 14px', background: 'rgba(0,0,0,0.6)', border: '1px solid var(--sand)', borderRadius: 6, color: 'var(--charcoal)', fontSize: 14, cursor: 'pointer' }}>{t.upload}</button>
                <button onClick={(e) => { e.stopPropagation(); updateHero({ img: '', imgCrop: null }) }} style={{ width: 28, height: 28, background: 'rgba(0,0,0,0.6)', border: '1px solid var(--sand)', borderRadius: 6, color: 'var(--rose)', fontSize: 17, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>&times;</button>
              </div>
            </>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--muted)', fontSize: 17 }}>{t.pfClickHeroUpload}</div>
          )}
        </div>
      ...
      <div style={panelStyle}>
        <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--charcoal)', marginBottom: 4 }}>{t.pfCardTitle}</h3>
        <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 20 }}>{t.pfCardDesc}</p>
        ...
        <div>
          ...
          <div className="admin-edit-grid" style={{ alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ color: 'var(--muted)', fontSize: 14 }}>{t.pfColor}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ color: 'var(--muted)', fontSize: 14, whiteSpace: 'nowrap' }}>{t.pfOpacity} {cardSettings.overlayOpacity}%</span>
      ...
      <div style={panelStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4, flexWrap: 'wrap', gap: 8 }}>
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--charcoal)' }}>{t.pfTagGroups}</h3>
          <button onClick={addPillGroup} style={{ padding: '6px 14px', background: 'var(--rose-soft)', color: 'var(--rose)', border: 'none', borderRadius: 8, fontSize: 15, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>{t.pfAddGroup}</button>
        </div>
        <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 20 }}>{t.pfTagGroupsDesc}</p>
            ...
            <div key={group.id} style={{ border: '1px solid var(--sand)', borderRadius: 10, padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                <input
                  ...
                  style={{ ...inputStyle, flex: 1, minWidth: 120, fontFamily: 'var(--font-serif)', fontSize: 20, borderTop: 'none', borderLeft: 'none', borderRight: 'none', borderRadius: 0, padding: '4px 0', background: 'transparent' }}
                />
                ...
                <span className="mob-hide" style={{ fontFamily: 'monospace', fontSize: 13, color: 'var(--muted)', background: 'var(--warm)', padding: '4px 8px', borderRadius: 4 }}>{group.dataKey}</span>
                <button onClick={() => removePillGroup(gi)} style={{ width: 28, height: 28, background: 'transparent', border: '1px solid var(--sand)', borderRadius: 6, color: 'var(--rose)', cursor: 'pointer', fontSize: 17, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>&times;</button>
              </div>
                ...
                {group.options.map((opt, oi) => (
                  <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'var(--warm)', borderRadius: 6, padding: '4px 8px' }}>
                    <input
                      ...
                      style={{ background: 'transparent', border: 'none', color: 'var(--charcoal)', fontSize: 15, fontFamily: 'var(--font-sans)', outline: 'none', width: Math.max(60, opt.length * 8) }}
                    />
                    <button onClick={() => removeOption(gi, oi)} style={{ background: 'transparent', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 14, padding: 0 }}>&times;</button>
                  </div>
                ))}
                <button onClick={() => addOption(gi)} style={{ padding: '4px 12px', border: '1px dashed var(--sand)', borderRadius: 6, background: 'transparent', color: 'var(--muted)', fontSize: 14, cursor: 'pointer' }}>{t.pfAddOption}</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={panelStyle}>
        <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--charcoal)', marginBottom: 4 }}>{t.pfFormTitle}</h3>
        <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 20 }}>{t.pfFormDesc}</p>
          ...
          {formConfig.fields.map((field, fi) => (
            <div key={field.id}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--warm)', borderRadius: 8, padding: '8px 12px', flexWrap: 'wrap' }}>
                <span className="mob-hide" style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', color: 'var(--muted)', letterSpacing: '0.05em', width: 60, flexShrink: 0 }}>{field.id}</span>
                <input value={field.label} onChange={e => updateFormField(fi, { label: e.target.value })} style={{ ...inputStyle, flex: 1, minWidth: 100, padding: '6px 10px', fontSize: 15 }} />
                <select value={field.type} onChange={e => updateFormField(fi, { type: e.target.value as FormField['type'] })} style={{ ...inputStyle, width: 120, padding: '6px 8px', fontSize: 14 }}>
                ...
                <select value={field.width} onChange={e => updateFormField(fi, { width: e.target.value as FormField['width'] })} style={{ ...inputStyle, width: 80, padding: '6px 8px', fontSize: 14 }}>
                  {['full', 'half', 'third'].map(w => (
                    <option key={w} value={w}>{widthLabels[w] || w}</option>
                  ))}
                </select>
                <button onClick={() => updateFormField(fi, { required: !field.required })} style={{ width: 28, height: 28, borderRadius: 6, border: 'none', background: field.required ? 'var(--rose)' : 'var(--sand)', color: field.required ? 'white' : 'var(--muted)', fontSize: 17, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>*</button>
                <input value={field.placeholder || ''} onChange={e => updateFormField(fi, { placeholder: e.target.value })} placeholder={t.pfFormFieldPh} style={{ ...inputStyle, width: 120, minWidth: 80, padding: '6px 10px', fontSize: 14 }} />
                <button onClick={() => removeFormField(fi)} style={{ width: 28, height: 28, background: 'transparent', border: '1px solid var(--sand)', borderRadius: 6, color: 'var(--rose)', cursor: 'pointer', fontSize: 17, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>&times;</button>
              </div>
              {field.type === 'file_upload' && (
                <div style={{ marginTop: 4, marginLeft: 0 }}>
                  <input value={field.helperText || ''} onChange={e => updateFormField(fi, { helperText: e.target.value })} placeholder={t.pfFormHelperPh} style={{ ...inputStyle, fontSize: 14, padding: '6px 10px' }} />
                </div>
              )}
            </div>
          ))}
        </div>
        <button onClick={addFormField} style={{ width: '100%', padding: '10px 0', border: '1px dashed var(--sand)', borderRadius: 8, background: 'transparent', color: 'var(--muted)', fontSize: 15, cursor: 'pointer' }}>{t.pfAddField}</button>
      </div>

      {/* ─── CATEGORY SECTIONS ─────────────────────────────── */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--charcoal)' }}>{t.pfProfileSections}</h3>
        <button onClick={addCategory} style={{ padding: '6px 14px', background: 'var(--rose-soft)', color: 'var(--rose)', border: 'none', borderRadius: 8, fontSize: 15, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>{t.pfAddSection}</button>
      </div>
        ...
        <div key={cat.id} style={panelStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <input
              ...
              style={{ fontFamily: 'var(--font-serif)', fontSize: 22, color: 'var(--charcoal)', background: 'transparent', border: 'none', borderBottom: '2px solid var(--sand)', padding: '4px 0', flex: 1, outline: 'none', minWidth: 0 }}
            />
            <button onClick={() => removeCategory(ci)} style={{ width: 28, height: 28, background: 'transparent', border: '1px solid var(--sand)', borderRadius: 6, color: 'var(--rose)', cursor: 'pointer', fontSize: 17, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>&times;</button>
          </div>
          <div className="admin-edit-grid" style={{ marginBottom: 12 }}>
            {cat.fields.map((field, fi) => (
              <div key={field.key} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--warm)', borderRadius: 6, padding: '6px 10px' }}>
                <span className="mob-hide" style={{ fontFamily: 'monospace', fontSize: 13, color: 'var(--muted)', width: 60, flexShrink: 0 }}>{field.key}</span>
                <input value={field.label} onChange={e => updateCategoryField(ci, fi, e.target.value)} style={{ ...inputStyle, flex: 1, padding: '4px 8px', fontSize: 15, minWidth: 0 }} />
                <button onClick={() => removeCategoryField(ci, fi)} style={{ background: 'transparent', border: 'none', color: 'var(--rose)', cursor: 'pointer', fontSize: 17, padding: 0, flexShrink: 0 }}>&times;</button>
              </div>
            ))}
          </div>
          <button onClick={() => addCategoryField(ci)} style={{ padding: '6px 14px', border: '1px dashed var(--sand)', borderRadius: 6, background: 'transparent', color: 'var(--muted)', fontSize: 14, cursor: 'pointer' }}>{t.pfAddField}</button>
        </div>
      ))}
    </div>
  )
