...
export default function ModelCard({ profile, cardSettings }: ModelCardProps) {
  ...
  const placeholderSvg = `data:image/svg+xml,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="600" viewBox="0 0 400 600"><rect width="400" height="600" fill="#0a0a0a"/><text x="200" y="280" text-anchor="middle" font-family="Georgia,serif" font-size="64" font-weight="300" fill="rgba(255,255,255,0.28)">${firstLetter}</text></svg>`
  )}`;
      ...
      <div className="relative overflow-hidden" style={{ paddingTop: '135%' }}>
        ...
        {profile.vacation && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/50">
            <span className="font-serif text-xl tracking-[0.2em] text-white uppercase px-5 py-2 border border-white/30 backdrop-blur-sm">
              Vacation
            </span>
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 flex gap-1.5 z-20">
          {cardSettings.showVerifiedBadge !== false && profile.verified && (
            <span className="font-sans bg-[rgba(0,0,0,0.9)] backdrop-blur-sm px-2.5 py-[3px] text-[11px] font-bold tracking-[0.08em] text-sage uppercase">
