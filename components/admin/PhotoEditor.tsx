    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(12px)' }}>
      <div className="sans" style={{ background: 'var(--card-bg)', maxWidth: 520, width: '90%', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--sand)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--charcoal)' }}>{t.editPhoto || 'Edit Photo'}</div>
          <button onClick={onCancel} style={{ background: 'none', border: '1px solid var(--sand)', width: 28, height: 28, cursor: 'pointer', fontSize: 17, color: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Manrope,sans-serif' }}>
            &times;
          </button>
        </div>
        ...
        <div style={{ padding: '20px 24px' }}>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 8 }}>
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--muted)', textTransform: 'uppercase' }}>{t.zoom || 'Zoom'}</span>
              <span style={{ fontSize: 14, color: 'var(--charcoal)', fontWeight: 600 }}>{zoom}%</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
            <div style={{ fontSize: 13, color: 'var(--muted)' }}>X: <strong style={{ color: 'var(--charcoal)' }}>{Math.round(pos.x)}%</strong></div>
            <div style={{ fontSize: 13, color: 'var(--muted)' }}>Y: <strong style={{ color: 'var(--charcoal)' }}>{Math.round(pos.y)}%</strong></div>
          </div>
        </div>
        ...
        <div style={{ padding: '12px 24px', borderTop: '1px solid var(--sand)', display: 'flex', gap: 10 }}>
          <button
            ...
            style={{ flex: 1, padding: '10px 0', background: 'var(--charcoal)', color: 'var(--cream)', border: 'none', fontSize: 13, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Manrope,sans-serif' }}
          >
            {t.applyCrop || 'Apply'}
          </button>
          <button
            onClick={() => { setPos({ x: 50, y: 50 }); setZoom(100); }}
            style={{ padding: '10px 16px', border: '1px solid var(--sand)', background: 'transparent', color: 'var(--muted)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Manrope,sans-serif' }}
          >
            {t.resetCrop || 'Reset'}
          </button>
          <button
